...
* All framework exceptions extend `TamboUIException` (`dev.tamboui.errors.TamboUIException`). This is a `RuntimeException`.
* Terminal I/O errors are wrapped in `RuntimeIOException` (`dev.tamboui.errors.RuntimeIOException`), to avoid having to catch `IOException` which often are non-recoverable.
