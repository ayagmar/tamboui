...
NOTE: **For Toolkit DSL users**: Use `RichList` instead (via `list()` factory method). It manages state internally and accepts any `StyledElement` as items. See <<listwidget-vs-listcontainer>>.
...
==== ListWidget vs RichList

TamboUI provides two list implementations at different API levels:

[cols="1,2,2",options="header"]
|===
|Feature |ListWidget (widgets layer) |RichList (toolkit layer)
...
**RichList** is for Toolkit DSL users who want:
...
[source,java]
----
// Toolkit DSL - RichList with rich content
list()
