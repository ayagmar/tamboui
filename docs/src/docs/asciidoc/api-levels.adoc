...
[source,java]
----
private RichList<?> myList = list("Apple", "Banana", "Cherry")
...
// In render():
myList  // RichList manages selection and scroll internally
...
RichList provides methods for navigation:
[source,java]
----
...
public class TodoApp extends ToolkitApp {
    ...
    private final RichList<?> todoList = list()
        .highlightColor(Color.CYAN)
        .autoScroll();
